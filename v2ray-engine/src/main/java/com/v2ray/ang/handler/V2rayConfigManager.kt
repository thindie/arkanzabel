...
object V2rayConfigManager {
    ...
    private fun getV2rayNormalConfig(context: Context, guid: String, config: ProfileItem): ConfigResult {
        ...
        v2rayConfig.log.loglevel = KeyValueStorage.decodeSettingsString(AppConfig.PREF_LOGLEVEL) ?: "warning"
        ...
        if (KeyValueStorage.decodeSettingsBool(AppConfig.PREF_SPEED_ENABLED) != true) {
    ...
    private fun updateOutboundWithGlobalSettings(outbound: OutboundBean): Boolean {
        try {
            ...
            if (muxEnabled == true) {
                ...
                outbound.mux?.concurrency = KeyValueStorage.decodeSettingsString(AppConfig.PREF_MUX_CONCURRENCY, "8").orEmpty().toInt()
                outbound.mux?.xudpConcurrency = KeyValueStorage.decodeSettingsString(AppConfig.PREF_MUX_XUDP_CONCURRENCY, "16").orEmpty().toInt()
                outbound.mux?.xudpProxyUDP443 = KeyValueStorage.decodeSettingsString(AppConfig.PREF_MUX_XUDP_QUIC, "reject")
            ...
            when (config.policyGroupType) {
                ...
                "1" -> {
                    ...
                    v2rayConfig.burstObservatory = V2rayConfig.BurstObservatoryObject(
                        ...
                        pingConfig = V2rayConfig.BurstObservatoryObject.PingConfigObject(
                            destination = KeyValueStorage.decodeSettingsString(AppConfig.PREF_DELAY_TEST_URL) ?: AppConfig.DELAY_TEST_URL,
                            interval = "5m",
                            sampling = 2,
                            timeout = "30s"
                        )
                ...
                "3" -> {
                    ...
                    v2rayConfig.observatory = V2rayConfig.ObservatoryObject(
                        subjectSelector = lstSelector,
                        probeUrl = KeyValueStorage.decodeSettingsString(AppConfig.PREF_DELAY_TEST_URL) ?: AppConfig.DELAY_TEST_URL,
                        probeInterval = "3m",
