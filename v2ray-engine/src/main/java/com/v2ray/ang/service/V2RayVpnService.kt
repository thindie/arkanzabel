...
class V2RayVpnService : VpnService(), ServiceControl {
    ...
    private fun configurePerAppProxy(builder: Builder) {
        ...
        if (KeyValueStorage.decodeSettingsBool(AppConfig.PREF_PER_APP_PROXY) == false) {
        ...
        val apps =
            KeyValueStorage.decodeSettingsStringSet(AppConfig.PREF_PER_APP_PROXY_SET)?.toMutableSet()
        if (apps.isNullOrEmpty()) {
            builder.addDisallowedApplication(selfPackageName)
            return
        }

        val bypassApps = KeyValueStorage.decodeSettingsBool(AppConfig.PREF_BYPASS_APPS)
