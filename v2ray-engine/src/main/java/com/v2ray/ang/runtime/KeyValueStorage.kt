...
object KeyValueStorage {
  ...
  fun encodeServerTestDelayMillis(
    guid: String,
    testResult: Long,
  ) {
    if (guid.isBlank()) return
  ...
  fun encodeSettings(
    key: String,
    value: Set<String>,
  ): Boolean = settingsStorage.encode(key, value.toMutableSet())
  ...
  fun decodeSettingsStringAsInt(
    key: String,
    defaultValue: Int,
  ): Int = decodeSettingsString(key)?.toIntOrNull() ?: defaultValue

  fun decodeSettingsInt(
    key: String,
    defaultValue: Int,
  ): Int = settingsStorage.decodeInt(key, defaultValue)

  fun decodeSettingsLong(
    key: String,
    defaultValue: Long,
  ): Long = settingsStorage.decodeLong(key, defaultValue)

  fun decodeSettingsFloat(
    key: String,
    defaultValue: Float,
  ): Float = settingsStorage.decodeFloat(key, defaultValue)
  ...
  fun decodeSettingsStringSet(key: String): Set<String>? = settingsStorage.decodeStringSet(key)?.toSet()
  ...
  fun encodeWebDavConfig(config: WebDavConfig): Boolean = mainStorage.encode(KEY_WEBDAV_CONFIG, JsonUtil.toJson(config))
  ...
  fun isVpnSessionActive(): Boolean = mainStorage.decodeBool(AppConfig.PREF_VPN_SESSION_ACTIVE, false)

  fun setVpnSessionStartEpochMs(epochMs: Long) {
    mainStorage.encode(AppConfig.PREF_VPN_SESSION_START_EPOCH_MS, epochMs)
  }

  fun getVpnSessionStartEpochMs(): Long = mainStorage.decodeLong(AppConfig.PREF_VPN_SESSION_START_EPOCH_MS, 0L)
  ...
  fun getLastAutoSaveNotificationTimestamp(): Long = mainStorage.decodeLong(KEY_LAST_AUTO_SAVE_NOTIFICATION_TIMESTAMP, 0L)

  fun setLastAutoSaveProfilesJson(profilesJson: String) {
    mainStorage.encode(KEY_LAST_AUTO_SAVE_PROFILES_JSON, profilesJson)
  }

  fun getLastAutoSaveProfilesJson(): String? = mainStorage.decodeString(KEY_LAST_AUTO_SAVE_PROFILES_JSON)

  fun getLocalProfiles(): String? = mainStorage.decodeString(KEY_SAVED_PROFILES_JSON)
