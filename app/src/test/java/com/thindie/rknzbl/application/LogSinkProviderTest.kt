...
class LogSinkProviderTest {
  private val kernelTag = AppConfig.TAG_KERNEL
  ...
  @Test
  fun kernel_log_routes_to_kernel_flow_only() =
    runTest {
  ...
  @Test
  fun debug_log_routes_to_debug_flow_only() =
    runTest {
  ...
  @Test
  fun mixed_logs_route_correctly() =
    runTest {
  ...
  @Test
  fun clear_by_level_removes_matching_entries() =
    runTest {
  ...
  @Test
  fun clear_kernel_by_level_removes_matching_entries() =
    runTest {
  ...
  @Test
  fun clear_debug_does_not_affect_kernel() =
    runTest {
  ...
  @Test
  fun clear_kernel_does_not_affect_debug() =
    runTest {
      Log.i({ "debug msg" }, debugTag)
      Log.i({ "kernel msg" }, kernelTag)
  ...
  @Test
  fun onEmitStatus_routes_to_kernel_flow() =
    runTest {
      // Reproduce the body of V2RayServiceManager's CoreCallbackHandler.onEmitStatus
      val code = 1L
      val msg: String? = "started"
      Log.i({ "core callback: onEmitStatus code=$code msg=${msg ?: "null"}" }, AppConfig.TAG_KERNEL)

      assertEquals(1, LogSinkProvider.kernelEntries.value.size)
      val entry = LogSinkProvider.kernelEntries.value.single()
      assertEquals(AppConfig.TAG_KERNEL, entry.tag)
      assertEquals(Log.Level.INFO, entry.level)
      assertEquals("core callback: onEmitStatus code=$code msg=$msg", entry.message)
      assertEquals(0, LogSinkProvider.entries.value.size)
    }
}
